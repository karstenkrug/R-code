...
######################################################
## plotly volcano
volc_ly <- function(fc, logP, fdr, id,
                    fc.min=1,  
                    fdr.max=0.01, 
                    cex=4,
                    main='',
                    alpha=100){
  require(plotly)
  dat <- data.frame(fc, logP, fdr, id,
                    cex=rep(cex, length(fc)),
                    info=paste(id,'\nFDR: ', round(fdr,3), '\n', sep=''))
  
  if(is.null(fc.min))
    sig <- which(fdr < fdr.max)
  else
    sig <- which(fdr < fdr.max & abs(fc) > fc.min)
  
  significant <- rep('n', length(fc))
  significant[sig] <- 'y'
  
  dat <- data.frame(dat, significant)
  #dat.bg <- dat[-sig, ]
  #dat.sig <- dat[sig, ]
  
  p <- ggplot(dat, aes(fc, logP, text=info) ) +
    geom_point(aes(colour=significant)) +
    geom_vline(xintercept=0, linetype=3) +
    labs(title=glue('{main}'), y='-log(P-value)', x='logFC') +
    scale_colour_manual(name="", values = c("y"=my.col2rgb("darkred", alpha = alpha), 
                                            "n"=my.col2rgb("grey", alpha = alpha)))
  if(!is.null(fc.min))
    p <- p +  geom_vline(xintercept=c(-fc.min, fc.min), linetype=3) 
...
## ###############################################################
## gsea_volc_ly
gsea_volc_ly <- function(fc, logP, fdr, id, perc, perc.scale=3, 
                         fdr.max=0.05, main=''){
  require(plotly)
...
#################################################################################################
##                 convert R color names to hexcode
## changelog: 20200707 implementaion
#####################################################################################################
col2hex <- function(col){
  rgb <- as.vector(col2rgb(col))
...
fancyBarplot <- function(x, space=0.2, 
                         ylim=c(0, max(x, na.rm=T)+ 0.2*max(x, na.rm=T)), 
                         ndigits=3, 
                         add.numb=T,
                      ...
                      xlim=NULL, ylim=NULL, ...)
{
  ...
  if(col.asis){
    ...
    col <- my.col2rgb(col, alpha) ## 20201203
  }
...
##
#############################################################################################################
timeProfileClusterPlot <- function(data, hc=NULL, norm=c("none", "Zscore"), log=T, logBase=2, plotOnRawScale=F, addAverageProfile=T, y.lim=NULL, algorithm=c("cmeans", "mycmeans", "kmeans", "consensus", "hclust"), nClust=3, file="clusterTimeProfiles.pdf", pdf=F, ylab="Change in expression", col=c("green", "magenta", "darkred"), nrows=1, minMembShip=0.5, cmeansm=2, alpha=40, legend=T, legend.pos=c('topright', 'bottom'), ...)
{
    ##alpha=80
    ...
    for( cluster in levels(as.factor(cl)) ){
        ...
        if((algorithm=="cmeans" | algorithm=="mycmeans") & legend)
            legend(legend.pos, legend=c( paste( "mean: ", round(mean(maxScore),2) ) , paste("median: ", round(median(maxScore),2)), paste("N <", minMembShip, ":", sum(maxScore < minMembShip)) ), title="Membership" )

        if(algorithm=="consensus" & legend)
            legend(legend.pos, legend=c( paste( "cluster consensus: ", round(clustering$clusterConsensus[as.numeric( cluster) ], 3) ) ) )

        axis(1, at=seq(1,dim(data)[2]), labels=colnames(data), las=1, cex.axis=2)
...
####################################################################################################################
my.cmeans <- function(x, centers=NULL, iter.max=1e5, maxK=NULL, verbose=FALSE, dist="euclidean", method="cmeans", m=NULL, rate.par=NULL, weights=1, control=list(reltol=(.Machine$double.eps))){
...
#######################################################################################
##
## pairs plot with adjustable xlim and ylim 
## https://stackoverflow.com/questions/22810309/pairs-specifying-axes-limits-of-the-subpanels
##
